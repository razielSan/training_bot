...
class Diary(Base):
    ...
    training_count: Mapped[int] = mapped_column(default=0)  # какая по счету тренировка в день
    ...
    total_repetition: Mapped[int] = mapped_column(default=0) # Общее количество повторений
    rest: Mapped[float]  # отдых
    exercise: Mapped[str] # название упражнения
    completed: Mapped[bool]  = mapped_column(default=False) # Флаг проверки завершенности упражнения
    ...
    calendar_day: Mapped["CalendarDays"] = relationship(back_populates="training_records")


    exercise_detail: Mapped[List["Exercise"]] = relationship(back_populates="diary", lazy='subquery')
